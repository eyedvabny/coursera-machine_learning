...
% Convert Y into a matrix of features
% [5000 10]
ident = eye(num_labels);
pred_vals = ident(y,:);

% mean(-[5000 10][5000 10] - (1-[5000 10])[5000 10])
J = mean(sum(-pred_vals.*log(a3) - (1-pred_vals).*log(1-a3),2));

% Compute regularization cost terms
reg_cost = lambda/2/m * (sum(sum(Theta1(:,2:end).^2,2)) + sum(sum(Theta2(:,2:end).^2,2)));
J = J + reg_cost;

% Backtrack on the gradients
...
for t=1:m
   ...
   % [1 26] = [1 10][10 26] .* [1 26]
   d2 = (d3*Theta2) .* [1, sigmoidGradient(z2(t,:))];
